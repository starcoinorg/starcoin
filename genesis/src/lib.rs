...
use starcoin_chain::migration::migrate_data_to_statedb;
    ...
    fn build_genesis_block(net: &ChainNetwork) -> Result<Block> {
        ...
        }) = genesis_config.genesis_block_parameter()
        {
            ...
            if net.is_main() || net.is_proxima() {
                debug!("Genesis::build_genesis_block | net is proxima or main, start migrating");

                // txn_info.state_root_hash is the state root generated by state db after write set has written.
                // After data migration has completed,
                // the state root of statedb has changed, so we need to update it
                txn_info.state_root_hash = migrate_data_to_statedb(&chain_state_db)?;
            }
...
mod tests {
    ...
    #[stest::test]
    pub fn test_genesis_main_for_migration() -> Result<()> {
        do_test_genesis(
            &ChainNetwork::new_builtin(Main),
            starcoin_config::temp_dir().path(),
            false,
        )?;
        Ok(())
    }
    ...
    pub fn do_test_genesis(net: &ChainNetwork, data_dir: &Path, legacy: bool) -> Result<()> {
        ...
        let capacity_size = DEFAULT_CACHE_SIZE * 10000;

        debug!("do_test_genesis | genesis check on storage 1");
        let storage1 = Arc::new(Storage::new(
            StorageInstance::new_cache_instance_with_capacity(capacity_size),
        )?);
        let storage2 = Arc::new(Storage2::new(StorageInstance2::new_cache_instance())?);
        ...
        let storage1_2 = Arc::new(Storage::new(
            StorageInstance::new_cache_instance_with_capacity(capacity_size),
        )?);
