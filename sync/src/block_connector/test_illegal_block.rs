...
async fn new_block_and_main() -> (Block, BlockChain) {
    ...
    let head_id = writeable_block_chain_service
        ...
        .current_header()
        .id();
    let main = BlockChain::new_v2(net.time_service(), head_id, storage, storage2, None).unwrap();
...
fn apply_with_illegal_uncle(
    ...
    let head_id = writeable_block_chain_service
        ...
        .current_header()
        .id();
    let mut main = BlockChain::new_v2(net.time_service(), head_id, storage, storage2, None)?;
...
async fn test_verify_can_not_be_uncle_check_ancestor_failed() {
    ...
    let tmp_head = writeable_block_chain_service
        ...
        .unwrap()
        .id();
    let mut new_branch = BlockChain::new_v2(
        net.time_service(),
        tmp_head,
        storage.clone(),
        storage2.clone(),
