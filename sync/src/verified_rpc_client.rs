...
use starcoin_types::transaction::{SignedUserTransactionV2, Transaction};
use starcoin_types::{
...
impl VerifiedRpcClient {
    ...
    pub async fn get_txns_with_hash_from_pool(
        ...
        req: GetTxnsWithHash,
    ) -> Result<(Vec<HashValue>, Vec<SignedUserTransactionV2>)> {
        ...
        if data.len() == req.len() {
            ...
            let mut verified_txns: Vec<SignedUserTransactionV2> = Vec::new();
