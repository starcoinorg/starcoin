...
fn test_get_blocks_multiple_blocks_with_retry() -> Result<()> {
    ...
    let parent_block = block_on(async { 
        chain_service.get_block_by_hash(parent_hash).await 
    })?.unwrap();
    
    info!("Head block: #{}, parent: #{}", head_block.header().number(), parent_block.header().number());
    ...
    let result = block_on(async {
        verified_client.get_blocks(vec![head_block.id(), parent_block.id()]).await
    })?;
    ...
    assert_eq!(ret_block1.id(), head_block.id(), "First block ID should match");
    assert_eq!(ret_block2.id(), parent_block.id(), "Second block ID should match");
    
    assert!(peer1.is_some(), "Peer info should be present");
    assert!(peer2.is_some(), "Peer info should be present");
    assert_eq!(peer1.as_ref().unwrap(), &peer_id2, "Block should come from peer_id2");
    assert_eq!(peer2.as_ref().unwrap(), &peer_id2, "Block should come from peer_id2");
