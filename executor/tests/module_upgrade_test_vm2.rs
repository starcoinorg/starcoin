...
#[stest::test]
fn test_simple_module_upgrade() -> Result<()> {
    let (chain_state, net) = prepare_genesis()?;
    let alice = Account::new();

    info!("Testing simple module upgrade scenario");

    // Create a simple upgrade proposal
    let dao_action_type_tag = UpgradeModuleV2::type_tag();

    let mock_package_hash = [1u8; 32];
    let proposal_id = 0u64;

    let vote_script_payload = dao_upgrade_module_proposal_propose_module_upgrade_v2(
        stc_type_tag(),
        genesis_address(),
        mock_package_hash.to_vec(),
        2,     // version 2
    ...
    info!("Simple module upgrade test completed successfully");
    Ok(())
}
