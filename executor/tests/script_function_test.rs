...
use starcoin_vm_types::transaction::{Package, ScriptFunction, TransactionPayload, Script};
use starcoin_vm_types::vm_status::KeptVMStatus;
use statedb::ChainStateDB;
use test_helper::executor::{compile_modules_with_address, execute_and_apply, prepare_genesis, compile_ir_script, compile_script};
...
fn test_execute_script() -> Result<()> {
    ...
    let script = prepare_script(SyntaxChoice::Source, 
        r#"
        script{
...
fn test_execute_script_verify() -> Result<()> {
    ...
    let script = prepare_script(SyntaxChoice::IR, 
        r#"
            main() {
                let v: vector<u8>;
            label b0:
