...
use starcoin_types::{account_address, multi_transaction::MultiSignedUserTransaction, transaction::SignedUserTransaction};
...
fn test_chain_get_block_txn_infos_in_seq() -> Result<()> {
    ...
    let import_result2 = txpool.add_txns_multi_signed(multi_txns, false, Some("test-client".to_string()))?;
    ...
    if let starcoin_rpc_api::types::TransactionInfoViewEnum::Vm2(first_txn) = &txn_infos_in_seq[0] {
        assert_eq!(first_txn.transaction_index, 0, "First transaction should have index 0");
    } else {
    ...
    for (i, info) in txn_infos_in_seq.iter().enumerate() {
        let txn_index = match info {
            starcoin_rpc_api::types::TransactionInfoViewEnum::Vm1(vm1_info) => vm1_info.transaction_index,
            starcoin_rpc_api::types::TransactionInfoViewEnum::Vm2(vm2_info) => vm2_info.transaction_index,
        };
        assert_eq!(txn_index as usize, i, "Transaction index should match position in array");
    }
    info!("✓ All transaction indices are sequential");
    
    let vm2_count = txn_infos_in_seq.iter().filter(|info| {
        matches!(info, starcoin_rpc_api::types::TransactionInfoViewEnum::Vm2(_))
    }).count();
    
    assert!(vm2_count >= 1, "Should have at least 1 VM2 transaction (block meta)");
    ...
    if !vm1_infos.is_empty() {
        let vm1_hashes: std::collections::HashSet<_> = vm1_infos.iter()
            .map(|info| info.transaction_hash)
            .collect();
        
        let seq_vm1_hashes: std::collections::HashSet<_> = txn_infos_in_seq.iter()
            .filter_map(|info| {
        ...
        assert_eq!(vm1_hashes, seq_vm1_hashes, "VM1 transaction hashes should match");
    }
    
    if !vm2_infos.is_empty() {
        let vm2_hashes: std::collections::HashSet<_> = vm2_infos.iter()
            .map(|info| info.transaction_hash)
            .collect();
        
        let seq_vm2_hashes: std::collections::HashSet<_> = txn_infos_in_seq.iter()
            .filter_map(|info| {
        ...
        assert_eq!(vm2_hashes, seq_vm2_hashes, "VM2 transaction hashes should match");
    }
