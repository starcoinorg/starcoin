...
// use starcoin_types::block::{BlockHeader, BlockHeaderBuilder, BlockHeaderExtra};
...
//         now = timestamp;
//         blocks.push_front(BlockDiffInfo2::new(
//             timestamp,
//             difficult_to_target(diff).unwrap(),
//             0,
//         ));
//         let bf: Vec<&BlockDiffInfo2> = blocks.iter().collect();
//         let blocks = bf.iter().map(|&b| b.clone()).collect::<Vec<_>>();
//         diff = target_to_difficulty(get_next_target_helper(blocks.clone(), time_plan).unwrap())
//             .unwrap();
//     }
...
//     let blocks = (0..24)
//         .rev()
//         .map(|i| BlockDiffInfo2 {
//             timestamp: time_plan * i,
//             target: target0,
//             transaction_count: 0,
//         })
//         .collect::<Vec<_>>();
//     let next_target = get_next_target_helper(blocks.clone(), time_plan).unwrap();
//     assert_eq!(next_target, target0);
// }
...
//     let next_target_1 = get_next_target_helper(blocks_1.clone(), time_plan).unwrap();
//     let next_target_2 = get_next_target_helper(blocks_2.clone(), time_plan).unwrap();
