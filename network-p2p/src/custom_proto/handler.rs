...
use bytes::BytesMut;
use futures::prelude::*;
use futures::{compat::Compat, TryFutureExt};
use futures_timer::Delay;
use libp2p::core::upgrade::{InboundUpgrade, OutboundUpgrade};
use libp2p::core::{ConnectedPoint, Endpoint, PeerId};
use libp2p::swarm::{
    protocols_handler::IntoProtocolsHandler, KeepAlive, ProtocolsHandler, ProtocolsHandlerEvent,
...
pub struct CustomProtoHandler<TSubstream> {
    ...
    events_queue: SmallVec<
        [ProtocolsHandlerEvent<RegisteredProtocol, (), CustomProtoHandlerOut, void::Void>; 16],
    >,
}
    ...
    TSubstream: AsyncRead + AsyncWrite,
{
    ...
    fn poll_state(
        &mut self,
    ) -> Option<ProtocolsHandlerEvent<RegisteredProtocol, (), CustomProtoHandlerOut, void::Void>>
    {
            ...
            ProtocolState::Normal {
                ...
                for n in (0..substreams.len()).rev() {
                    ...
                    match substream.poll() {
                        Ok(Poll::Pending) => substreams.push(substream),
                        Ok(Poll::Ready(Some(RegisteredProtocolEvent::Message(message)))) => {
                        ...
                        Ok(Poll::Ready(Some(RegisteredProtocolEvent::Clogged { messages }))) => {
    ...
    'outer: for n in (0..list.len()).rev() {
        ...
        loop {
            match substream.poll() {
                ...
                Err(_) | Ok(Poll::Ready(None)) => continue 'outer,
            }
        }
        list.push(substream);
    }
