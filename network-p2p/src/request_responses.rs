...
pub use network_p2p_types::{IncomingRequest, RequestFailure, ResponseFailure};
...
mod tests {
    ...
    fn basic_request_response_works() {
        ...
        async_std::task::spawn({
            ...
            async move {
                loop {
                    if let SwarmEvent::Behaviour(super::Event::InboundRequest { result, .. }) =
                ...
                let behaviour = {
                    ...
                    async_std::task::spawn(async move {
                        while let Some(rq) = rx.next().await {
                            ...
                            let _ = rq
                                .pending_response
                                .send(b"this response exceeds the limit".to_vec());
                        }
                    });
            ...
            loop {
                match swarm.next_event().await {
                    ...
                    SwarmEvent::Behaviour(super::Event::RequestFinished { request_id, result }) => {
                        assert_eq!(Some(request_id), sent_request_id);
                        match result {
                            Err(super::RequestFailure::Network(
                                super::OutboundFailure::ConnectionClosed,
                            )) => {}
                            _ => panic!(),
                        }
                        break;
