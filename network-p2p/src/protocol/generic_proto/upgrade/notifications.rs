    ...
    TSubstream: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    fn upgrade_inbound(self, mut socket: TSubstream, _: Self::Info) -> Self::Future {
        Box::pin(async move {
...
mod tests {
    ...
    fn basic_works() {
        ...
        async_std::task::block_on(async move {
            ...
            let (initial_message, mut substream) =
                upgrade::apply_inbound(socket, NotificationsIn::new(PROTO_NAME))
                    .await
                    .unwrap();
    ...
    fn large_initial_message_refused() {
        ...
        async_std::task::block_on(async move {
            ...
            let (socket, _) = listener.accept().await.unwrap();
            let ret = upgrade::apply_inbound(socket, NotificationsIn::new(PROTO_NAME)).await;
            assert!(ret.is_err());
        });
    ...
    fn buffer_is_full_closes_connection() {
        ...
        let client = async_std::task::spawn(async move {
            ...
            for _ in 0..32768 {
                ...
                if future::poll_fn(|cx| Sink::poll_ready(Pin::new(&mut substream), cx))
                    .await
                    .is_err()
                {
                    return Ok(());
