...
use crate::protocol::generic_proto::handler::{
    NotificationsSink, NotifsHandler, NotifsHandlerIn, NotifsHandlerOut, NotifsHandlerProto,
};
use bytes::BytesMut;
use fnv::FnvHashMap;
use futures::prelude::*;
use libp2p::core::{transport::PortUse, ConnectedPoint, Endpoint, Multiaddr, PeerId};
use libp2p::swarm::behaviour::{ConnectionClosed, ConnectionEstablished, FromSwarm};
use libp2p::swarm::dial_opts::DialOpts;
use libp2p::swarm::{
    dial_opts::PeerCondition, ConnectionDenied, ConnectionId, NetworkBehaviour, NotifyHandler,
...
impl GenericProto {
    ...
    fn disconnect_peer_inner(
        ...
        match mem::replace(entry.get_mut(), PeerState::Poisoned) {
            ...
            PeerState::DisabledPendingEnable {
                ...
                self.peerset
                    .dropped(set_id, *peer_id, sc_peerset::DropReason::Unknown);
    ...
    pub fn requested_peers(&self, set_id: sc_peerset::SetId) -> impl Iterator<Item = &PeerId> {
        self.peers
            .iter()
    ...
    pub fn write_notification(
        ...
        let notifs_sink = match self
            .peers
            .get(&(*target, set_id))
            .and_then(|p| p.get_open())
        {
            None => {
    ...
    fn peerset_report_connect(&mut self, peer_id: PeerId, set_id: sc_peerset::SetId) {
        ...
        match mem::replace(occ_entry.get_mut(), PeerState::Poisoned) {
            ...
            PeerState::Disabled {
                ...
                if let Some((connec_id, connec_state)) = connections
                    .iter_mut()
                ...
                match mem::replace(entry.get_mut(), PeerState::Poisoned) {
                    PeerState::Enabled { mut connections } => {
                        ...
                        if !connections.iter().any(|(_, s)| {
                            ...
                            self.peerset
                                .dropped(set_id, source, sc_peerset::DropReason::Refused);
