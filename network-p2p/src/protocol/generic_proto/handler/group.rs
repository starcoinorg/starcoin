...
use libp2p::core::upgrade::{InboundUpgrade, OutboundUpgrade, SelectUpgrade, UpgradeError};
...
impl NotificationsSink {
    ...
    /// The protocol name is expected to be checked ahead of calling this method. It is a logic
    /// error to send a notification using an unknown protocol.
    /// note: this lifetimes is not needless, but clippy report warning.
...
impl ProtocolsHandler for NotifsHandler {
    ...
    fn inject_fully_negotiated_inbound(
        ...
        match out {
            EitherOutput::First((out, num)) => self.in_handlers[num]
                .0
    ...
    fn inject_fully_negotiated_outbound(
        ...
        self.out_handlers[num]
            .0
            .inject_fully_negotiated_outbound(out, ())
    }
        ...
        match message {
            ...
            NotifsHandlerIn::Disable => {
                ...
                for num in self.pending_in.drain(..) {
                    self.in_handlers[num]
                        .0
                        .inject_event(NotifsInHandlerIn::Refuse);
                }
            }
        }
    }
    ...
    fn inject_dial_upgrade_error(
        ...
        match err {
            ProtocolsHandlerUpgrErr::Timeout => self.out_handlers[num]
                .0
                .inject_dial_upgrade_error((), ProtocolsHandlerUpgrErr::Timeout),
            ProtocolsHandlerUpgrErr::Timer => self.out_handlers[num]
                .0
    ...
    > {
                ...
                match ev {
                    ProtocolsHandlerEvent::OutboundSubstreamRequest { protocol } => {
                        return Poll::Ready(ProtocolsHandlerEvent::OutboundSubstreamRequest {
                            protocol: protocol.map_info(|()| handler_num),
                        })
                    }
                    ProtocolsHandlerEvent::Close(err) => void::unreachable(err),
        ...
        if self
            .out_handlers
            .iter()
            .all(|(h, _)| h.is_open() || h.is_refused())
        {
