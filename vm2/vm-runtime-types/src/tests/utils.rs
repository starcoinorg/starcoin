...
pub(crate) fn mock_module_modify(_k: impl ToString, v: u128) -> (StateKey, WriteOp) {
    ...
    (StateKey::module_id(&dummy_module_id), write_op)
}
...
pub(crate) fn mock_add(k: impl ToString, v: u128) -> (StateKey, DeltaOp) {
    ...
    (
        as_state_key!(k),
        DeltaOp::new(SignedU128::Positive(v), DUMMY_LIMIT, history),
    )
}
...
impl ExpandedVMChangeSetBuilder {
    ...
    pub(crate) fn try_build(self) -> starcoin_vm2_vm_types::errors::PartialVMResult<VMChangeSet> {
