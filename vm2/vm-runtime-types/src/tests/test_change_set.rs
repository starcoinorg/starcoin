use crate::{
    ...
    tests::utils::{
        ...
        raw_metadata, ExpandedVMChangeSetBuilder, VMChangeSetBuilder,
    },
};
use move_core_types::{
...
use starcoin_vm2_types::delayed_fields::{PanicError, SnapshotToStringFormula};
...
#[test]
fn test_failed_squash() {
    // modify + create leads to error
    let write_set_1 = vec![mock_modify_with_layout("4", 4, None)];
    let write_set_2 = vec![mock_create_with_layout("4", 104, None)];

    let mut cs1 = VMChangeSetBuilder::new()
        .with_resource_write_set(write_set_1)
        .build();
    let cs2 = VMChangeSetBuilder::new()
        .with_resource_write_set(write_set_2)
        .build();
    let res = cs1.squash_additional_change_set(cs2);
    let err = assert_err!(res);
    assert!(
        err.major_status() == StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR
            || err.major_status() == StatusCode::DELAYED_MATERIALIZATION_CODE_INVARIANT_ERROR
    );
}
...
fn test_write_and_read_discrepancy_caught() {
    assert_err!(ExpandedVMChangeSetBuilder::new()
        ...
        .with_reads_needing_delayed_field_exchange(vec![(
            as_state_key!("1"),
            (StateValueMetadata::none(), 10, Arc::new(MoveTypeLayout::U64))
        )])
        .try_build());
