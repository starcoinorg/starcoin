...
use starcoin_vm2_vm_types::{
    identifier::IdentStr as IdentStr2, state_store::table::TableHandle as TableHandle2,
};
use starcoin_vm2_vm_types::{
    language_storage::StructTag as StructTag2, state_store::state_key::StateKey as StateKey2,
};
...
impl RemoteRpcAsyncClient {
    ...
    pub async fn get_modules_async(
        ...
        let state = self
            ...
            .get_account_state_set(
                AccountAddress2::from_bytes(addr.into_bytes()).unwrap(),
    ...
    pub async fn get_module_async(&self, module_id: &ModuleId) -> VMResult<Option<Vec<u8>>> {
        let state_key = StateKey2::module(
            &AccountAddress2::from_bytes(module_id.address().into_bytes()).unwrap(),
            IdentStr2::new(module_id.name().as_str()).unwrap(),
        );
    ...
    pub async fn get_resource_async(
        ...
        let state_key = StateKey2::resource(
            &AccountAddress2::from_bytes(address.into_bytes()).unwrap(),
            &StructTag2::from_str(tag.to_canonical_string().as_str()).unwrap(),
        )
        .unwrap();
        let state_with_proof = self
            .state_client
    ...
    pub async fn resolve_table_entry_async(
        ...
        let handle = TableHandle2(AccountAddress2::from_bytes(handle.0.into_bytes()).unwrap());
