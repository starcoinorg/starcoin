    ...
    fn try_from(txn: StcTransaction) -> Result<Self, Self::Error> {
        Ok(match txn {
            StcTransaction::V1(txn) => {
                match txn {
                    Transaction::UserTransaction(user_txn) => MultiSignedUserTransaction::VM1(user_txn),
                    _ => return Err(format_err!("Not a user transaction")),
                }
            }
            StcTransaction::V2(txn) => {
                match txn {
                    Transaction2::UserTransaction(user_txn) => MultiSignedUserTransaction::VM2(user_txn),
                    _ => return Err(format_err!("Not a user transaction")),
                }
            }
        })
