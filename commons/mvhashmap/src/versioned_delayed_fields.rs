...
use dashmap::DashMap;
use starcoin_aggregator::types::{code_invariant_error, PanicOr};
...
impl<K: Copy + Clone + Debug + Eq> VersionedValue<K> {
    ...
    fn remove(&mut self, txn_idx: TxnIndex, is_blockstm_v2: bool) {
        ...
        let deleted_entry = deleted_entry.expect("Entry must exist to be removed");
        let deleted = deleted_entry.as_ref().deref();
        assert!(
            matches!(deleted, VersionEntry::Estimate(_)),
...
pub struct VersionedDelayedFields<K: Clone> {
    ...
    ids_by_txn: DashMap<TxnIndex, HashSet<K>>,
...
impl<K: Eq + Hash + Clone + Debug + Copy> VersionedDelayedFields<K> {
    ...
    pub fn initialize_delayed_field(
        ...
        if let Some(mut existing) = self.values.get_mut(&id) {
            let same_txn = self
                .ids_by_txn
                .get(&txn_idx)
                .map(|ids| ids.contains(&id))
                .unwrap_or(false);
            if same_txn && existing.base_value.is_none() {
                existing.insert_speculative_value(txn_idx, VersionEntry::Value(value, None))?;
                return Ok(());
            }
    ...
    pub fn initialize_dependent_delayed_field(
        ...
        if let Some(mut existing) = self.values.get_mut(&id) {
            let same_txn = self
                ...
                .unwrap_or(false);
            if same_txn && existing.base_value.is_none() {
                existing.insert_speculative_value(txn_idx, VersionEntry::Apply(apply))?;
                return Ok(());
            }
            return Err(code_invariant_error(
                "VersionedValue when initializing dependent delayed field may not already exist for same id",
            ));
        }
    ...
    pub fn record_change(
        ...
        self.track_change(txn_idx, id);
        Ok(())
    }

    pub fn take_txn_ids(&self, txn_idx: TxnIndex) -> HashSet<K> {
        self.ids_by_txn
            .remove(&txn_idx)
            .map(|(_, ids)| ids)
            .unwrap_or_default()
    ...
    pub fn get_txn_ids(&self, txn_idx: TxnIndex) -> HashSet<K> {
        self.ids_by_txn
            .get(&txn_idx)
    ...
    pub fn remove(
        ...
        if let Some(mut entry) = self.ids_by_txn.get_mut(&txn_idx) {
            entry.remove(id);
            if entry.is_empty() {
                drop(entry);
                self.ids_by_txn.remove(&txn_idx);
            }
        }
        Ok(())
    }

    fn track_change(&self, txn_idx: TxnIndex, id: K) {
        let mut entry = self.ids_by_txn.entry(txn_idx).or_default();
        entry.insert(id);
    }
    ...
    pub fn try_commit(
        ...
        let mut ids_iter = ids_iter.peekable();
        if ids_iter.peek().is_none() {
        ...
        for id in ids_iter {
            let mut versioned_value = self.values.get_mut(&id).ok_or_else(|| {
                CommitError::CodeInvariantError(format!(
                    "Value in commit needs to be in the HashMap for id {:?}",
                    id
                ))
            })?;
...
mod test {
    ...
    use starcoin_vm2_types::delayed_fields::SnapshotToStringFormula;
    ...
    #[test]
    fn try_commit_missing_entry_returns_error() {
        ...
        let err = fields
            .try_commit(0, std::iter::once(id))
            .expect_err("missing entry should return error");
