...
pub(crate) fn result_to_json(result: Result<Value>) -> Value {
    match result {
        Ok(value) => {
            json!({ "ok": value })
        }
...
pub fn print_action_result(format: OutputFormat, result: &Value) -> Result<()> {
    match format {
...
pub fn print_table(value: &Value) -> Result<()> {
    ...
    match value {
        Value::Array(values) => print_vec_table(values.as_slice()),
        value => print_value_table(value),
    }
}

fn print_vec_table(values: &[Value]) -> Result<()> {
    if values.is_empty() {
        return Ok(());
    }
    let first = &values[0];
