...
use jsonrpsee::{
    ...
    BatchResponse, BoundedSubscriptions, MethodResponse, MethodSink, Methods,
};
use std::{
    future::Future,
    io,
        ...
        + 'static,
{
    ...
    /// use starcoin_rpc_ipc::server::Builder;
...
where
    ...
    for<'a> <RpcMiddleware as Layer<RpcService>>::Service: Send
        + Sync
        + 'static
        + RpcServiceT<MethodResponse = MethodResponse, BatchResponse = BatchResponse>,
{
    ...
    fn call(&mut self, request: String) -> Self::Future {
        ...
        let f = tokio::task::spawn(async move {
            ipc::call_with_service(request, rpc_service, max_request_body_size, conn).await
        });
