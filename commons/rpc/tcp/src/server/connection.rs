...
pub(crate) struct TcpConn<T>(#[pin] pub(crate) T);

impl<T> Stream for TcpConn<JsonRpcStream<T>>
where
...
impl<T> Sink<String> for TcpConn<JsonRpcStream<T>>
where
    T: AsyncRead + AsyncWrite,
{
    type Error = io::Error;
...
pub(crate) struct TcpConnDriver<T, S, Fut> {
    #[pin]
    pub(crate) conn: TcpConn<JsonRpcStream<T>>,
    pub(crate) service: S,
    #[pin]
...
impl<T, S, Fut> TcpConnDriver<T, S, Fut> {
    pub(crate) fn push_back(&mut self, item: String) {
        self.items.push_back(item);
    }
}

impl<T, S> Future for TcpConnDriver<T, S, S::Future>
where
                    ...
                    Ok(()) => {
                        if let Some(item) = this.items.pop_front() {
                            if let Err(err) = this.conn.as_mut().start_send(item) {
                                tracing::warn!("TCP response failed: {:?}", err);
                                return Poll::Ready(());
                            }
                        } else {
                            break;
            ...
            'inner: loop {
                ...
                match this.service.poll_ready(cx) {
                    ...
                    Poll::Ready(Err(err)) => {
                        ...
                        tracing::warn!(error = %err, "TCP request dispatch failed");
                        return Poll::Ready(());
                    }
                ...
                match this.conn.as_mut().poll_next(cx) {
                    Poll::Ready(res) => match res {
                        ...
                        Some(Err(err)) => {
                            tracing::debug!("TCP request failed: {:?}", err);
                            return Poll::Ready(());
                        }
