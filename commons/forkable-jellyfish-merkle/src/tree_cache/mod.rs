...
struct FrozenTreeCache<K: RawKey> {
    /// Immutable node_cache.
    node_cache: BTreeMap<NodeKey, Node<K>>,
...
impl<K> Default for FrozenTreeCache<K>
where
    K: RawKey,
{
    fn default() -> Self {
        Self {
            ...
            num_new_leaves: 0,
            stale_node_index_cache: BTreeSet::default(),
            num_stale_leaves: 0,
            root_hashes: vec![],
        }
    }
...
pub struct TreeCache<'a, R: 'a + TreeReader<K>, K: RawKey> {
    ...
    frozen_cache: FrozenTreeCache<K>,

    /// The underlying persistent storage.
    reader: &'a R,
}

impl<'a, R, K> TreeCache<'a, R, K>
where
    R: 'a + TreeReader<K>,
    K: RawKey,
{
    ...
    pub fn get_node(&self, node_key: &NodeKey) -> Result<Node<K>> {
    ...
    #[cfg(test)]
    pub fn get_root_node(&self) -> Result<Node<K>> {
    ...
    pub fn put_node(&mut self, node_key: NodeKey, new_node: Node<K>) -> Result<()> {
...
where
    ...
    K: RawKey,
{
    fn into(self) -> (Vec<HashValue>, TreeUpdateBatch<K>) {
        (
