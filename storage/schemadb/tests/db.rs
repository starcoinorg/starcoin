...
fn get_column_families() -> Vec<ColumnFamilyName> {
    vec![
        ...
        TestSchema1::COLUMN_FAMILY,
        TestSchema2::COLUMN_FAMILY,
    ]
}

fn open_db(dir: &starcoin_temppath::TempPath) -> DB {
    let db_opts = RocksdbConfig::default();
    DB::open("test", dir.path(), get_column_families(), db_opts, None).expect("Failed to open DB.")
}

fn open_db_read_only(dir: &starcoin_temppath::TempPath) -> DB {
    DB::open_readonly(
        "test",
        dir.path(),
        get_column_families(),
...
fn open_db_as_secondary(
    dir: &starcoin_temppath::TempPath,
    dir_sec: &starcoin_temppath::TempPath,
) -> DB {
...
struct TestDB {
    _tmpdir: starcoin_temppath::TempPath,
    db: DB,
}

impl TestDB {
    fn new() -> Self {
        let tmpdir = starcoin_temppath::TempPath::new();
...
fn test_open_as_secondary() {
    let tmpdir = starcoin_temppath::TempPath::new();
    let tmpdir_sec = starcoin_temppath::TempPath::new();
...
//fn test_checkpoint() {
//    let tmpdir = starcoin_temppath::TempPath::new();
//    let checkpoint = starcoin_temppath::TempPath::new();
//    {
//        let db = open_db(&tmpdir);
//        db.put::<TestSchema1>(&TestField(0), &TestField(0)).unwrap();
