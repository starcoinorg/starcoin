...
fn test_db_batch() {
    ...
    write_batch
        .put::<HashValue, TransactionInfo>(DEFAULT_CF_NAME, id, transaction_info1.clone())
        .unwrap();
    ...
    write_batch
        .put::<HashValue, TransactionInfo>(DEFAULT_CF_NAME, id2, transaction_info2.clone())
        .unwrap();
    db_storage.write_batch(write_batch).unwrap();
    assert_eq!(
        TransactionInfo::decode_value(
            &db_storage
                .get(DEFAULT_CF_NAME, id.to_vec())
                .unwrap()
                .unwrap()
        )
        .unwrap(),
    ...
    assert_eq!(
        TransactionInfo::decode_value(
            &db_storage
                .get(DEFAULT_CF_NAME, id2.to_vec())
                .unwrap()
                .unwrap()
        )
        .unwrap(),
