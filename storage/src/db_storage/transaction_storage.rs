...
use rocksdb::{
    DBIteratorWithThreadMode, DBPinnableSlice, IteratorMode, OptimisticTransactionDB, Options,
...
pub struct DBTransactionStorage {
    db: OptimisticTransactionDB,
    cfs: Vec<ColumnFamilyName>,
    metrics: Option<StorageMetrics>,
}

impl DBTransactionStorage {
    ...
    pub fn open_with_cfs(
        ...
        let db = Self::open_inner(&rocksdb_opts, path, column_families.clone())?;
    ...
    fn open_inner(
        ...
        let inner = rocksdb::OptimisticTransactionDB::open_cf_descriptors(
            opts,
            path,
    ...
    ) -> Result<TransactionSchemaIterator<K, V>>
    where
        K: KeyCodec,
        V: ValueCodec,
    {
        let cf_handle = self.get_cf_handle(prefix_name);
        Ok(TransactionSchemaIterator::new(
            self.db
    ...
    ) -> Result<DBIteratorWithThreadMode<'_, OptimisticTransactionDB>> {
        let cf_handle = self.get_cf_handle(prefix_name);
        Ok(self.db.iterator_cf_opt(cf_handle, readopts, mode))
    }

    /// Returns a forward [`SchemaIterator`] on a certain schema.
    pub fn iter<K, V>(&self, prefix_name: &str) -> Result<TransactionSchemaIterator<K, V>>
    where
        K: KeyCodec,
        V: ValueCodec,
    {
    ...
    pub fn rev_iter<K, V>(&self, prefix_name: &str) -> Result<TransactionSchemaIterator<K, V>>
    where
        K: KeyCodec,
        V: ValueCodec,
    {
...
pub struct TransactionSchemaIterator<'a, K, V> {
    db_iter: rocksdb::DBRawIteratorWithThreadMode<'a, rocksdb::OptimisticTransactionDB>,
    direction: ScanDirection,
    phantom_k: PhantomData<K>,
    phantom_v: PhantomData<V>,
}

impl<'a, K, V> TransactionSchemaIterator<'a, K, V>
where
    K: KeyCodec,
    V: ValueCodec,
{
...
impl<'a, K, V> Iterator for TransactionSchemaIterator<'a, K, V>
where
    K: KeyCodec,
    V: ValueCodec,
{
...
impl InnerStore for DBTransactionStorage {
...
impl RawDBStorage<true> for DBTransactionStorage {
    ...
    fn raw_write_batch(&self, batch: WriteBatchWithTransaction<true>) -> Result<()> {
        self.db.write(batch)?;
        Ok(())
    }
}
