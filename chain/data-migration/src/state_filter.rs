...
use starcoin_vm_types::{
    ...
    on_chain_resource::{BlockMetadata, Epoch, GlobalTimeOnChain},
...
pub fn filter_chain_state_set(
    ...
    let association_address = association_address();
    for (address, account_state_set) in chain_state_set.state_sets() {
        ...
        if let Some(resource_set) = account_state_set.resource_set() {
            for (key, blob) in resource_set.iter() {
                ...
                let filtered_blob = match (
                    struct_tag.address == genesis_address,
                    struct_tag.module.as_str(),
                    struct_tag.name.as_str(),
                ) {
                    (true, AccountResource::MODULE_NAME, AccountResource::STRUCT_NAME) => {
                        if *address == association_address {
                            // Replace association's authentication key
                            let origin_ar = statedb
                                .get_account_resource(*address)?
                                .expect("Account resource must exist");
                            let readed_account_resource =
                                bcs_ext::from_bytes::<AccountResource>(blob)?
                                    .clone_with_parameters(
                                        0,
                                        Some(origin_ar.authentication_key().to_vec()),
                                    );
                            bcs_ext::to_bytes(&readed_account_resource)?
                        } else {
                            bcs_ext::to_bytes(
                                &bcs_ext::from_bytes::<AccountResource>(blob)?
                                    .clone_with_parameters(0, None),
                            )?
                        }
                    }

                    (true, BlockMetadata::MODULE_NAME, BlockMetadata::STRUCT_NAME) => {
                        bcs_ext::to_bytes(&statedb.get_block_metadata()?)?
                    }

                    (true, "BlockReward", "RewardQueue") => {
                    ...
                    (true, ChainId::MODULE_NAME, ChainId::STRUCT_NAME) => {
                        bcs_ext::to_bytes(&statedb.get_chain_id()?)?
                    }

                    (true, GlobalTimeOnChain::MODULE_NAME, GlobalTimeOnChain::STRUCT_NAME) => {
                        bcs_ext::to_bytes(&statedb.get_timestamp()?)?
                    }

                    (true, Epoch::MODULE_NAME, Epoch::STRUCT_NAME) => {
                        bcs_ext::to_bytes(&statedb.get_epoch()?)?
                    }
