...
use starcoin_types::{contract_event::ContractEvent, event::EventKey};
use std::{
        ...
        match resource_path.file_stem() {
            ...
            Some(name) => Ok({
                ...
                match Self::get_bytes(resource_path)? {
                    Some(resource_data) => {
                        Some(MoveValueAnnotator::new(self).view_struct(id, &resource_data)?)
                    }
                    None => None,
                }
            }),
    ...
    fn get(&self, access_path: &AccessPath) -> Result<Option<Vec<u8>>> {
        match &access_path.path {
            DataPath::Code(module_name) => {
                let module_id = ModuleId::new(access_path.address, module_name.clone());
                Ok(self
                    .get_module(&module_id)
                    .map_err(|e| e.into_vm_status())?)
            }
            DataPath::Resource(struct_tag) => Ok(self
                .get_resource(&access_path.address, struct_tag)
