...
/// Content for starcoin framework directory
const DIR_STARCOIN_FRAMEWORK: Dir = include_dir!("../stdlib/modules");
...
static PACKAGE_STARCOIN_FRAMEWORK: Lazy<MovePackage> = Lazy::new(|| MovePackage {
    name: "starcoin",
    sources: vec![SourceFilter {
        source_dir: &DIR_STARCOIN_FRAMEWORK,
        inclusion: None, // include everything
        exclusion: HashSet::new(),
    }],
    deps: vec![],
    ...
    fn prepare(&self, out_path: &Path, source_only: bool) -> Result<Vec<String>> {
        ...
        if !source_only && !pkg_bin_path.exists() {
            ...
            let (_files, compiled_units) =
                Compiler::new(&[path_to_string(&pkg_src_path)?], &src_dirs).build_and_report()?;
