...
pub use starcoin_vm_types::aggregator::{DelayedFieldID, PanicError, TryFromMoveValue, TryIntoMoveValue};
...
impl<T: std::fmt::Debug> From<&PanicOr<T>> for StatusCode {
    fn from(err: &PanicOr<T>) -> Self {
        match err {
            // XXX FIXME YSG
            PanicOr::CodeInvariantError(_) => StatusCode::/*DELAYED_FIELDS_CODE_INVARIANT_ERROR*/DELAYED_MATERIALIZATION_CODE_INVARIANT_ERROR,
    ...
    fn from(err: PanicOr<T>) -> Self {
        match err {
            ...
            PanicOr::CodeInvariantError(msg) => {
                PartialVMError::new(StatusCode::/*DELAYED_FIELDS_CODE_INVARIANT_ERROR*/DELAYED_MATERIALIZATION_CODE_INVARIANT_ERROR)
                    .with_message(msg)
            },
