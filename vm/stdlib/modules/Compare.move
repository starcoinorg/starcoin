...
module Compare {
    ...
    //
    // This function is designed to compare SCS (Starcoin Canonical Serialization)-encoded values
    // (i.e., vectors produced by `SCS::to_bytes`). A typical client will call
    // `Compare::cmp_scs_bytes(SCS::to_bytes(&t1), SCS::to_bytes(&t2)). The comparison provides the
    // following guarantees w.r.t the original values t1 and t2:
    // - `cmp_scs_bytes(scs(t1), scs(t2)) == LESS_THAN` iff `cmp_scs_bytes(t2, t1) == GREATER_THAN`
    ...
    //   (`u8`, `u64`, `u128`), we have
    //   `compare_scs_bytes(scs(t1), scs(t2)) == LESS_THAN` iff `t1 < t2` and (similarly)
    //   `compare_scs_bytes(scs(t1), scs(t2)) == LESS_THAN` iff `t1 > t2`.
    //
    // For all other types, the order is whatever the SCS encoding of the type and the comparison
    // strategy above gives you. One case where the order might be surprising is the `address` type.
    // CoreAddresses are 16 byte hex values that SCS encodes with the identity function. The right to
    // left, byte-by-byte comparison means that (for example)
    // `compare_scs_bytes(scs(0x01), scs(0x10)) == LESS_THAN` (as you'd expect), but
    // `compare_scs_bytes(scs(0x100), scs(0x001)) == LESS_THAN` (as you probably wouldn't expect).
    // Keep this in mind when using this function to compare addresses.
    public fun cmp_scs_bytes(v1: &vector<u8>, v2: &vector<u8>): u8 {
        let i1 = Vector::length(v1);
        let i2 = Vector::length(v2);
        let len_cmp = cmp_u64(i1, i2);

        // SCS uses little endian encoding for all integer types, so we choose to compare from left
