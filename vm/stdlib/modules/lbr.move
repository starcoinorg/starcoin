...
module LBR {
    ...
    // restrictions are enforced in the Coin::register_currency function.
    public fun initialize() {
        ...
        let mint_cap = Coin::grant_mint_capability();
        let burn_cap = Coin::grant_burn_capability();
        let preburn_cap = Coin::new_preburn_with_capability(&burn_cap);
        let coin1 = ReserveComponent<Coin1::T> {
            ratio: FixedPoint32::create_from_rational(1, 2),
            backing: Coin::zero<Coin1::T>(),
        };
        let coin2 = ReserveComponent<Coin2::T> {
            ratio: FixedPoint32::create_from_rational(1, 2),
            backing: Coin::zero<Coin2::T>(),
        };
    ...
    public fun swap_into(
        coin1: Coin::T<Coin1::T>,
        coin2: Coin::T<Coin2::T>
    ): (Coin::T<T>, Coin::T<Coin1::T>, Coin::T<Coin2::T>)
    acquires Reserve {
        let reserve = borrow_global_mut<Reserve>(0xA550C18);
        let coin1_value = Coin::value(&coin1);
        let coin2_value = Coin::value(&coin2);
        if (coin1_value <= 1 || coin2_value <= 1) return (Coin::zero<T>(), coin1, coin2);
    ...
    acquires Reserve {
        if (amount_lbr == 0) return (Coin::zero<T>(), coin1, coin2);
        let reserve = borrow_global_mut<Reserve>(0xA550C18);
        ...
        let coin1_exact = Coin::withdraw(&mut coin1, num_coin1);
        let coin2_exact = Coin::withdraw(&mut coin2, num_coin2);
        Coin::deposit(&mut reserve.coin1.backing, coin1_exact);
        Coin::deposit(&mut reserve.coin2.backing, coin2_exact);
        (Coin::mint_with_capability<T>(amount_lbr, &reserve.mint_cap), coin1, coin2)
    }

    // Unpack a LBR coin and return the backing currencies (in the correct
    // amounts).
    public fun unpack(coin: Coin::T<T>): (Coin::T<Coin1::T>, Coin::T<Coin2::T>)
    acquires Reserve {
        let reserve = borrow_global_mut<Reserve>(0xA550C18);
        let ratio_multiplier = Coin::value(&coin);
        let sender = Transaction::sender();
        Coin::preburn_with_resource(coin, &mut reserve.preburn_cap, sender);
        Coin::burn_with_resource_cap(&mut reserve.preburn_cap, sender, &reserve.burn_cap);
        ...
        let coin1 = Coin::withdraw(&mut reserve.coin1.backing, coin1_amount);
        let coin2 = Coin::withdraw(&mut reserve.coin2.backing, coin2_amount);
        (coin1, coin2)
    }
    ...
    fun to_libra_value(coin1: &Coin::T<Coin1::T>): u64
    acquires Reserve {
        let coin1_value = Coin::value(coin1);
