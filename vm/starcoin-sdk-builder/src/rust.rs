...
/// If `local_types` is true, we generate a file suitable for the Starcoin codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targeting the Starcoin repository itself (as opposed to generated Starcoin types).
    local_types: bool,
}
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Starcoin `Script` from a structured object `ScriptCall`.
