...
use dashmap::DashMap;
use std::{collections::btree_map::BTreeMap, hash::Hash, sync::{
...
impl<K: Hash + Clone + Eq, V> MVHashMap<K, V> {
    ...
    pub fn write(&self, key: &K, version: Version, data: V) {
        ...
        println!("{:?} - MVHashMap::mark_estimate | Entered, version: {:?}", thread::current().id(), version);
        ...
        println!("{:?} - MVHashMap::mark_estimate | Exited, version: {:?}", thread::current().id(), version);
    }

    /// Mark an entry from transaction 'txn_idx' at access path 'key' as an estimated write
    /// (for future incarnation). Will panic if the entry is not in the data-structure.
    pub fn mark_estimate(&self, key: &K, txn_idx: TxnIndex) {
        println!("{:?} - MVHashMap::mark_estimate | Entered, txn_id: {:?}", thread::current().id(), txn_idx);
        ...
        println!("{:?} - MVHashMap::mark_estimate | Exited", thread::current().id());
    }
    ...
    pub fn delete(&self, key: &K, txn_idx: TxnIndex) {
        println!("{:?} - MVHashMap::delete | Exit, txn_idx: {:?}", thread::current().id(), txn_idx);

        // TODO: investigate logical deletion.
        let mut map = self.data.get_mut(key).expect("Path must exist");
        map.remove(&txn_idx);

        println!("{:?} - MVHashMap::delete | Exited, txn_idx: {:?}", thread::current().id(), txn_idx);
    }
