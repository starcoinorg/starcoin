...
module starcoin_framework::vesting {
    ...
    use starcoin_framework::starcoin_account::{Self, assert_account_is_registered_for_stc};
    ...
    public fun create_vesting_contract(
        ...
        buy_ins: SimpleMap<address, Coin<STC>>,
        vesting_schedule: VestingSchedule,
        withdrawal_address: address,
        operator: address,
        voter: address,
    ...
    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<STC> {
        ...
        let withdrawn_coins = coin::balance<STC>(contract_address);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        coin::withdraw<STC>(contract_signer, withdrawn_coins)
    }
    ...
    public fun setup_vesting_contract_with_schedule(
        ...
        let admin_address = signer::address_of(admin);
        let buy_ins = simple_map::create<address, Coin<STC>>();
    ...
    public entry fun test_end_to_end(
        ...
        distribute(contract_address);
        let shareholder_1_bal = coin::balance<STC>(shareholder_1_address);
        let shareholder_2_bal = coin::balance<STC>(shareholder_2_address);
        ...
        assert!(coin::balance<STC>(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(coin::balance<STC>(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(coin::balance<STC>(withdrawal_address) == 1, 0);
        ...
        let previous_bal = coin::balance<STC>(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(coin::balance<STC>(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }
    ...
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        assert!(coin::balance<STC>(shareholder_address) == staker_rewards + 1, 0);
        assert!(coin::balance<STC>(operator_address) == commission - 1, 1);
    }
    ...
    public entry fun test_commission_percentage_change(
        ...
        assert!(coin::balance<STC>(operator_address) == expected_commission, 1);
    }
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        assert!(coin::balance<STC>(operator_address1) == 0, 1);
        assert!(coin::balance<STC>(beneficiary_address) == expected_commission, 1);
        let old_beneficiay_balance = coin::balance<STC>(beneficiary_address);
        ...
        assert!(coin::balance<STC>(operator_address2) >= expected_commission, 1);
        assert!(coin::balance<STC>(beneficiary_address) == old_beneficiay_balance, 1);
    }
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        let balance = coin::balance<STC>(@11);
    ...
    public entry fun test_reset_beneficiary(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(coin::balance<STC>(@11) == vested_amount, 0);
        assert!(coin::balance<STC>(@12) == 0, 1);
    }
