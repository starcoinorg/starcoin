spec starcoin_framework::vesting {
    ...
    spec create_vesting_contract {
        ...
        aborts_if !exists<coin::CoinStore<STC>>(withdrawal_address);
    ...
    spec set_beneficiary(
        ...
        aborts_if !coin::spec_is_account_registered<STC>(new_beneficiary);
        include VerifyAdminAbortsIf;
    ...
    spec create_vesting_contract_account(
        ...
        aborts_if !exists<coin::CoinStore<STC>>(resource_addr) && !starcoin_std::type_info::spec_is_struct<STC>();
        aborts_if !exists<coin::CoinStore<STC>>(resource_addr) && ea && acc.guid_creation_num + 2 > MAX_U64;
        aborts_if !exists<coin::CoinStore<STC>>(resource_addr) && ea && acc.guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        ensures exists<account::Account>(resource_addr) && post_acc.authentication_key == account::ZERO_AUTH_KEY &&
                exists<coin::CoinStore<STC>>(resource_addr);
    ...
    spec withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<STC> {
        // TODO: Calls `staking_contract::distribute` which is not verified.
        pragma verify = false;
