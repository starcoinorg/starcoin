spec starcoin_framework::state_storage {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, the resources for tracking state storage usage and gas
    /// parameters must exist for the Starcoin framework address.
    /// Criticality: Critical
    /// Implementation: The initialize function ensures only the Starcoin framework address can call it.
    ...
    /// storage usage will be moved under the Starcoin framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the starcoin_framework account.
    ...
    /// usage will be moved under the Starcoin framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the starcoin_framework account.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-5.3]
        invariant [suspendable] chain_status::is_operating() ==> exists<StateStorageUsage>(@starcoin_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<GasParameter>(@starcoin_framework);
    }
    ...
    spec initialize(starcoin_framework: &signer) {
        ...
        let post state_usage = global<StateStorageUsage>(@starcoin_framework);
        /// [high-level-req-2]
    ...
    spec on_new_block(epoch: u64) {
        ...
        aborts_if false;
        ensures epoch == global<StateStorageUsage>(@starcoin_framework).epoch;
    }
