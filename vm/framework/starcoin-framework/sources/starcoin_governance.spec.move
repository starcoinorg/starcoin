spec starcoin_framework::starcoin_governance {
    ...
    spec store_signer_cap(
        ...
        aborts_if exists<GovernanceResponsbility>(@starcoin_framework) &&
            simple_map::spec_contains_key(signer_caps, signer_address);
        ensures exists<GovernanceResponsbility>(@starcoin_framework);
        let post post_signer_caps = global<GovernanceResponsbility>(@starcoin_framework).signer_caps;
    ...
    spec initialize_partial_voting(
        ...
        aborts_if exists<VotingRecordsV2>(@starcoin_framework);
        ensures exists<VotingRecordsV2>(@starcoin_framework);
    }
    ...
    spec update_governance_config(
        ...
        let addr = signer::address_of(starcoin_framework);
        let governance_config = global<GovernanceConfig>(@starcoin_framework);

        let post new_governance_config = global<GovernanceConfig>(@starcoin_framework);
        aborts_if addr != @starcoin_framework;
        aborts_if !exists<GovernanceConfig>(@starcoin_framework);
        aborts_if !exists<GovernanceEvents>(@starcoin_framework);
    ...
    spec has_entirely_voted(stake_pool: address, proposal_id: u64): bool {
        aborts_if !exists<VotingRecords>(@starcoin_framework);
    }
    ...
    /// Address @starcoin_framework must exist GovernanceEvents.
    spec schema CreateProposalAbortsIf {
        ...
        let staking_config = global<staking_config::StakingConfig>(@starcoin_framework);
        ...
        aborts_if !exists<voting::VotingForum<GovernanceProposal>>(@starcoin_framework);
        let voting_forum = global<voting::VotingForum<GovernanceProposal>>(@starcoin_framework);
        let proposal_id = voting_forum.next_proposal_id;
        aborts_if proposal_id + 1 > MAX_U64;
        let post post_voting_forum = global<voting::VotingForum<GovernanceProposal>>(@starcoin_framework);
    ...
    spec schema VoteAbortIf {
        ...
        aborts_if spec_proposal_expiration <= locked_until && !exists<timestamp::CurrentTimeMicroseconds>(@starcoin_framework);
        let spec_proposal_expiration = voting::spec_get_proposal_expiration_secs<GovernanceProposal>(@starcoin_framework, proposal_id);
        ...
        aborts_if !remain_zero_1_cond && !exists<VotingRecords>(@starcoin_framework);
        ...
        aborts_if !exists<VotingRecords>(@starcoin_framework);
        let voting_records = global<VotingRecords>(@starcoin_framework);


        // verify get_voting_power(stake_pool)
        let allow_validator_set_change = global<staking_config::StakingConfig>(@starcoin_framework).allow_validator_set_change;
        let stake_pool_res = global<stake::StakePool>(stake_pool);
        ...
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@starcoin_framework);

        // Due to the complexity of the success state, the validation of 'borrow_global_mut<ApprovedExecutionHashes>(@starcoin_framework);' is discussed in four cases.
        /// [high-level-req-3]
        aborts_if
            if (should_pass) {
                proposal_state_successed_0 && !exists<ApprovedExecutionHashes>(@starcoin_framework)
            } else {
                proposal_state_successed_1 && !exists<ApprovedExecutionHashes>(@starcoin_framework)
            };
        aborts_if
            if (should_pass) {
                proposal_state_successed_2 && !exists<ApprovedExecutionHashes>(@starcoin_framework)
            } else {
                proposal_state_successed_3 && !exists<ApprovedExecutionHashes>(@starcoin_framework)
            };
        ...
        aborts_if features::spec_partial_governance_voting_enabled() && !exists<VotingRecordsV2>(@starcoin_framework);
    }
    ...
    spec schema AddApprovedScriptHash {
        proposal_id: u64;
        aborts_if !exists<ApprovedExecutionHashes>(@starcoin_framework);
    ...
    spec resolve(proposal_id: u64, signer_address: address): signer {
        ...
        aborts_if !exists<ApprovedExecutionHashes>(@starcoin_framework);
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@starcoin_framework).hashes;
        ensures !simple_map::spec_contains_key(post_approved_hashes, proposal_id);

        // verify get_signer
        include GetSignerAbortsIf;
        let governance_responsibility = global<GovernanceResponsbility>(@starcoin_framework);
    ...
    spec remove_approved_hash(proposal_id: u64) {
        ...
        let proposal = table::spec_get(voting_forum.proposals, proposal_id);
        aborts_if !proposal.is_resolved;
        let post approved_hashes = global<ApprovedExecutionHashes>(@starcoin_framework).hashes;
    ...
    spec schema GetVotingPowerAbortsIf {
        ...
        aborts_if !allow_validator_set_change && !exists<stake::ValidatorSet>(@starcoin_framework);
    ...
    spec schema GetSignerAbortsIf {
        ...
        aborts_if !exists<GovernanceResponsbility>(@starcoin_framework);
        let cap_map = global<GovernanceResponsbility>(@starcoin_framework).signer_caps;
    ...
    spec force_end_epoch(starcoin_framework: &signer) {
        use starcoin_framework::reconfiguration_with_dkg;
        use std::signer;
        pragma verify = false; // TODO: set because of timeout (property proved).
        let address = signer::address_of(starcoin_framework);
        include reconfiguration_with_dkg::FinishRequirement {
            framework: starcoin_framework
        };
    }
