...
module alice::MyToken {
    ...
    use starcoin_framework::MintDaoProposal;
...
script {
    ...
    fun transfer_some_token_to_bob(signer: signer) {
        ...
        coin::transfer<MyToken>(&signer, @bob, balance / 2);
    }
}
// check: EXECUTED
...
script {
    ...
    fun vote(signer: signer) {
        let balance = account::balance<MyToken>(signer::address_of(&signer));
        let balance = coin::withdraw<MyToken>(&signer, balance);
...
script {
    ...
    fun queue_proposal(account: signer) {
        ...
        {
            let token = dao::unstake_votes<MyToken, MintDaoProposal::MintToken>(&account, @alice, 0);
