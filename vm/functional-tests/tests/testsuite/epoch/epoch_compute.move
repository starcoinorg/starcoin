script {
    ...
    fun compute_next_block_time_target() {
        ...
        let total_uncles = 0;
        let now_seconds = epoch_block_count*base_block_time_target;
        let last_block_time_target = base_block_time_target;
        assert(Epoch::compute_next_block_time_target(&config, last_block_time_target, 0, now_seconds,  0, epoch_block_count, total_uncles) < base_block_time_target, 101);

        let total_uncles = epoch_block_count * uncle_rate_target /1000;
        let new_block_time_target = Epoch::compute_next_block_time_target(&config, last_block_time_target, 0, now_seconds,  0, epoch_block_count, total_uncles);
        ...
        let total_uncles = 0;
        let now_seconds = epoch_block_count*base_block_time_target;
        assert(Epoch::compute_next_block_time_target(&config,last_block_time_target, 0, now_seconds,  0, epoch_block_count, total_uncles) < base_block_time_target, 103);
        ...
        let total_uncles = epoch_block_count*2-1;
        let now_seconds = epoch_block_count*last_block_time_target;
        let new_block_time_target = Epoch::compute_next_block_time_target(&config, last_block_time_target , 0, now_seconds,  0, epoch_block_count, total_uncles);
        Debug::print(&new_block_time_target);
        ...
        let total_uncles = 0;
        let now_seconds = epoch_block_count*min_block_time_target;
        let new_block_time_target = Epoch::compute_next_block_time_target(&config, last_block_time_target , 0, now_seconds,  0, epoch_block_count, total_uncles);
        Debug::print(&new_block_time_target);
