...
module SortedLinkedList {
    ...
    public fun insert_node<T: copyable>(account: &signer, data: T, prev_entry: EntryHandle) acquires NodeVector {
        ...
        let data_bcs_bytes = SCS::to_bytes(&data);
        let cmp_with_prev = Compare::cmp_scs_bytes(&data_bcs_bytes, &SCS::to_bytes(&prev_data));
        let cmp_with_next = Compare::cmp_scs_bytes(&data_bcs_bytes, &SCS::to_bytes(&next_data));
    ...
    public fun find_position_and_insert<T: copyable>(account: &signer, data: T, head: EntryHandle): bool acquires NodeVector {
        ...
        while (!Self::is_head_node<T>(&next_entry)) {
            ...
            let next_data_bcs_bytes = SCS::to_bytes(&next_node_data);
            let cmp = Compare::cmp_scs_bytes(&next_data_bcs_bytes, &data_bcs_bytes);
