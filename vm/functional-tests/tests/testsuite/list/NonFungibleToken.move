...
module SortedLinkedList {
    ...
    public fun add_node<T: copyable>(account: &signer, key: T, prev_node_address: address) acquires Node {
        ...
        let key_bcs_bytes = SCS::to_bytes(&key);
        let cmp_with_prev = Compare::cmp_scs_bytes(&key_bcs_bytes, &SCS::to_bytes(&prev_key));
        let cmp_with_next = Compare::cmp_scs_bytes(&key_bcs_bytes, &SCS::to_bytes(&next_key));
    ...
    public fun find<T: copyable>(key: T, head_address: address): (bool, address) acquires Node {
        ...
        while (next_node_address != head_address) {
            ...
            let next_key_bcs_bytes = SCS::to_bytes(&next_node_key);
            let cmp = Compare::cmp_scs_bytes(&next_key_bcs_bytes, &key_bcs_bytes);
