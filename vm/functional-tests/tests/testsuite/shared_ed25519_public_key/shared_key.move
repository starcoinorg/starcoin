...
fun main(account: &signer) {
    let old_auth_key = Account::authentication_key({{default}});
    // from RFC 8032
    ...
    let new_auth_key = Account::authentication_key({{default}});
    ...
    assert(Account::delegated_key_rotation_capability({{default}}), 3002);
    ...
    assert(new_auth_key != Account::authentication_key({{default}}), 3005);
}
}
// check: EXECUTED
