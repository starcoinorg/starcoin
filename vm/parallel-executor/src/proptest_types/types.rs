...
use starcoin_vm_types::{access_path::AccessPath, account_address::AccountAddress};
use std::{
    ...
    fn module_path(&self) -> Option<AccessPath> {
        if self.1 {
            Some(AccessPath {
                ...
                // XXX FIXME YSG CONFIRM
                path: AccessPath::code_data_path(Identifier::new("foo").unwrap()),
            })
        } else {
            None
        }
