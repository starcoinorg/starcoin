...
impl StarcoinVM {
    ...
    fn check_gas(&self, txn_data: &TransactionMetadata) -> Result<(), VMStatus> {
        ...
        // transaction.
        if txn_data.max_gas_amount().map(|gas| gas * gas_constants.gas_unit_scaling_factor).get() > gas_constants.maximum_number_of_gas_units.get() {
            warn!(
        ...
        if txn_data.max_gas_amount().map(|gas| gas * gas_constants.gas_unit_scaling_factor).get() < min_txn_fee.get() {
            warn!(
