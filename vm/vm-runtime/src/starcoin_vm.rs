...
impl StarcoinVM {
    ...
    fn verify_transaction_impl<S: MoveResolverExt + StateView>(
        ...
        match transaction.payload() {
            TransactionPayload::Package(package) => {
                ...
                let enforced = match Self::is_enforced(storage, package.package_address()) {
                    Ok(is_enforced) => is_enforced,
                    _ => false,
                };
                let only_new_module =
                    match Self::only_new_module_strategy(storage, package.package_address()) {
                        Err(e) => {
    ...
    pub fn verify_transaction<S: StateView>(
        ...
        match self.verify_transaction_impl(&signature_verified_txn, &data_cache.as_move_resolver())
        {
            Ok(_) => None,
            Err(err) => {
    ...
    fn only_new_module_strategy<S: MoveResolverExt + StateView>(
        storage: &S,
        package_address: AccountAddress,
    ) -> Result<bool> {
        let strategy_access_path = access_path_for_module_upgrade_strategy(package_address);
        if let Some(data) = storage.get_state_value(&StateKey::AccessPath(strategy_access_path))? {
    ...
    fn is_enforced<S: MoveResolverExt + StateView>(
        storage: &S,
        package_address: AccountAddress,
    ) -> Result<bool> {
        let chain_id = storage.get_chain_id()?;
        let block_meta = storage.get_block_metadata()?;
            ...
            || (chain_id.is_barnard() && block_meta.number < 8311392)
        {
            ...
            if let Some(data) =
                storage.get_state_value(&StateKey::AccessPath(two_phase_upgrade_v2_path))?
            {
    ...
    fn execute_package<S: MoveResolverExt + StateView>(
        ...
        }
        {
            ...
            let enforced = match Self::is_enforced(storage, package_address) {
                Ok(is_enforced) => is_enforced,
                _ => false,
            };
            let only_new_module = match Self::only_new_module_strategy(storage, package_address) {
                Err(e) => {
                    warn!("[VM]Update module strategy deserialize err : {:?}", e);
    ...
    /// in the `ACCOUNT_MODULE` on chain.
    fn run_epilogue<S: MoveResolverExt>(
        &self,
        session: &mut SessionAdapter<S>,
        gas_meter: &mut StarcoinGasMeter,
        txn_data: &TransactionMetadata,
    ...
    fn execute_user_transaction<S: MoveResolverExt + StateView>(
        ...
        let mut session: SessionAdapter<_> = self
            .move_vm
            .new_session(storage, SessionId::txn(*txn))
            .into();
            ...
            Ok(txn) => {
                let result = match txn.payload() {
                    ...
                    TransactionPayload::Package(p) => {
                        self.execute_package(storage, &mut gas_meter, &txn_data, p)
                    }
                };
                match result {
    ...
    pub fn dry_run_transaction<S: StateView>(
        ...
        let storage = remote_cache.as_move_resolver();
        let mut session : SessionAdapter<_> = self.move_vm.new_session_with_extensions();_
        let mut gas_meter = StarcoinGasMeter::new(gas_params.clone(), txn_data.max_gas_amount());
        gas_meter.set_metering(false);
    ...
    fn failed_transaction_cleanup<S: MoveResolverExt>(
        ...
        gas_meter.set_metering(false);
        /*
        let data_cache = storage.as_move_resolver(); */
        let mut session: SessionAdapter<_> = self
            .move_vm
            .new_session(&storage, SessionId::txn_meta(txn_data))
            .into();
