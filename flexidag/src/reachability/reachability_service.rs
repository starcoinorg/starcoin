...
pub trait ReachabilityService {
    ...
    fn is_dag_ancestor_of_any_result(
        &self,
        this: Hash,
    ...
    fn is_dag_ancestor_of_any_result(
        ...
        queried.try_fold(false, |acc, descendant| {
            if acc {
                ...
                inquirer::is_dag_ancestor_of(read_guard.deref(), this, descendant)
                    .map(|is_ancestor| acc || is_ancestor)
            }
        })
    }
...
impl<T: ReachabilityStoreReader + ?Sized> MTReachabilityService<T> {
    ...
    pub fn default_backward_chain_iterator(&self, from: Hash) -> impl Iterator<Item = Hash> + use<T> {
