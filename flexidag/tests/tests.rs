...
#[ignore = "Not sure aboout the test!"]
#[test]
fn test_dag_uncles() -> anyhow::Result<()> {
    // initialzie the dag firstly
    let mut dag = BlockDAG::create_for_testing().unwrap();
    ...
    parents_hash = vec![header_2];
    parent_hash = header_2;

    let header_3_1 = add_and_print(
        3,
        parent_hash,
        parents_hash.clone(),
        genesis.parent_hash(),
    ...
    let tips = [header_3_1.clone(), header_3_2.clone(), header_3_3.clone()];

    let max_difficulty_header = tips
        .iter()
        .max_by(|a, b| {
            let a_difficulty = a.difficulty();
            let b_difficulty = b.difficulty();
            a_difficulty.cmp(&b_difficulty)
        })
        .ok_or_else(|| format_err!("failed to get max difficulty header"))?;

    let sorted_tips = tips
        .iter()
        .sorted_by(|a, b| {
            let a_difficulty = a.difficulty();
            let b_difficulty = b.difficulty();
    ...
    sorted_tips.iter().for_each(|header| {
        println!(
            "sorted header: {:?}, difficulty: {:?}",
            header.id(),
            header.difficulty()
        );
    });
    ...
    println!(
        ...
        max_difficulty_header.id(),
        max_difficulty_header.difficulty()
    );

    let ghostdata = dag.ghostdata(
        &tips
            .into_iter()
            .map(|header| {
                println!(
    ...
    println!(
        "blue set: {:?}, red set: {:?}, blue anticone size: {:?}",
        ghostdata.mergeset_blues, ghostdata.mergeset_reds, ghostdata.blues_anticone_sizes
    );

    let header_3_4 = add_and_print(
        3,
        parent_hash,
        parents_hash.clone(),
        genesis.parent_hash(),
    ...
    let first_bluesets = ghostdata.mergeset_blues.clone();

    let tips_again = [header_3_1, header_3_2, header_3_3, header_3_4];
    ...
    let second_bluesets = ghostdata.mergeset_blues.clone();

    for index in 0..first_bluesets.len() {
        println!("check index: {:?}", index);
        assert_eq!(first_bluesets[index], second_bluesets[index]);
    }
