Feature: cmd integration test
  ...
  Scenario Outline: [cmd] cli chain test
    ...
    Then cmd: "chain get-txn-info-in-seq {{$.chain[1].ok[0].block_hash}}"
    Then cmd: "chain get-txn-proof --block-hash {{$.chain[1].ok[0].block_hash}} --transaction-global-index {{$.chain[9].ok[0].transaction_global_index}} --final-transaction-info-id {{$.chain[9].ok[-1].transaction_info_id}} --final-transaction-info-index {{$.chain[9].ok[-1].transaction_global_index}}"
    Then cmd: "chain get-txn-proof --block-hash {{$.chain[1].ok[0].block_hash}} --transaction-global-index {{$.chain[9].ok[0].transaction_global_index}} --raw --final-transaction-info-id {{$.chain[9].ok[-1].transaction_info_id}} --final-transaction-info-index {{$.chain[9].ok[-1].transaction_global_index}}"
    Then stop
  ...
  Scenario Outline: [cmd] multisig account
    ...
    Then cmd: "account transfer --blocking -r {{$.account[-1].ok.receipt_identifier}} -t 0x1::starcoin_coin::STC -v 10000000 --gas-token 0x1::starcoin_coin::STC"
  ...
  Scenario Outline: [cmd] dev resolve test
    Then cmd: "dev resolve function 0x1::transfer_scripts::peer_to_peer_v2"
  ...
  Scenario Outline: [cmd] account test
    ...
    Then cmd: "account rotate-authentication-key 0x809c795045105a7b1efbcca4510d2034 -i 0x3885e7dde8381046849d64d28b675f1c668dc36eaa9be11cbcaadb24c3917554 --gas-token 0x1::starcoin_coin::STC"
...
#      |  |
#
   #update_reward_config dao testing
  Scenario Outline: [cmd] starcoin RewardConfig dao
     # 1. deposit to default account which is a proposer
    Then cmd: "dev get-coin -v 1000000"
    Then cmd: "account unlock"
     # 2. create RewardConfig proposal with proposer account
    Then cmd: "account execute-function --function 0x1::on_chain_config_scripts::propose_update_reward_config -s {{$.account[0].ok.address}} --arg 10u64 --arg 0u64 -b"
    Then cmd: "dev sleep -t 60000"
     # 3. make sure proposal has been ACTIVE for voting
    Then cmd: "dev gen-block"
    Then cmd: "dev call --function 0x1::dao::proposal_state -t 0x1::starcoin_coin::STC -t 0x1::on_chain_config_dao::OnChainConfigUpdate<0x1::block_reward_config::RewardConfig> --arg {{$.account[0].ok.address}} --arg 0"
    ...
    Then cmd: "account execute-function --function 0x1::dao_vote_scripts::cast_vote -t 0x1::starcoin_coin::STC -t 0x1::on_chain_config_dao::OnChainConfigUpdate<0x1::block_reward_config::RewardConfig> -s {{$.account[2].ok.address}} --arg {{$.account[0].ok.address}} --arg 0 --arg true --arg 12740545600000000u128 -b"
    ...
    Then cmd: "account execute-function -s {{$.account[0].ok.address}} --function 0x1::dao::queue_proposal_action -t 0x1::starcoin_coin::STC -t 0x1::on_chain_config_dao::OnChainConfigUpdate<0x1::block_reward_config::RewardConfig> --arg {{$.account[0].ok.address}} --arg 0 -b"
    Then cmd: "dev sleep -t 3600000"
    ...
    Then cmd: "account execute-function -s {{$.account[0].ok.address}} --function 0x1::on_chain_config_scripts::execute_on_chain_config_proposal -t 0x1::block_reward_config::RewardConfig --arg 0 -b"
