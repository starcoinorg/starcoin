...
impl Miner {
    ...
    pub fn set_mint(&self, block_template: BlockTemplate, difficulty: U256) {
        ...
        *self.state.lock().unwrap() = Some(ctx);
        self.bus.do_send(Broadcast {
            msg: MintBlockEvent::new(header_hash, difficulty),
        });
    }
    ...
    pub fn submit(&self, nonce: u64, header_hash: HashValue) -> Result<()> {
        let ctx = self.state.lock().unwrap().take().ok_or_else(|| format_err!("Empty mine ctx"))?;
