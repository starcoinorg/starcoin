    ...
    P: TemplateTxProvider,
{
    ...
    pub fn create_block_template(&self) -> Result<BlockTemplateResponse> {
        ...
        let tips_hash = if current_number > self.chain.dag_fork_height() {
            let (_dag_genesis, tips_hash) = self.chain.current_tips_hash(&previous_header)?.ok_or(anyhow!("the number of the block is larger than the dag fork number but no dag state!"))?;
            Some(tips_hash)
        } else {
            None
        };
