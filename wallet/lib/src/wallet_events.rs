...
#[derive(Clone)]
pub struct WalletEventActor {
    bus: Addr<BusActor>,
    storage: WalletStorage,
}

impl WalletEventActor {
    pub fn launch(bus: Addr<BusActor>, storage: WalletStorage) -> Addr<Self> {
        WalletEventActor { bus, storage }.start()
    }
}

impl Actor for WalletEventActor {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        self.bus.clone()
            ...
            .then(|res, act, ctx| {
                match res {
                    Err(e) => {
                        error!(target: "wallet-events", "fail to start event subscription actor, err: {}", &e);
                        ctx.terminate();
                    }
                    Ok(r) => {
                        ctx.add_stream(r);
...
impl actix::StreamHandler<ContractEventNotification> for WalletEventActor {
