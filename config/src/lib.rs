    ...
    fn load_genesis_config_by_opt(
        ...
        let genesis_config = match (configs_in_file, id) {
            ...
            (None, ChainNetworkID::Custom(_net)) => {
                ...
                let mut genesis_configs =
                    match BuiltinNetworkID::from_str(config_name_or_path.as_str()) {
                ...
                genesis_configs.0.association_key_pair = (
                    Some(Arc::new(association_private_key)),
                    association_public_key,
                );

                let (association_private_key, association_public_key) =
                    genesis_multi_key_pair_vm2();
                genesis_configs.1.association_key_pair = (
                    Some(Arc::new(association_private_key)),
                    association_public_key,
                );
